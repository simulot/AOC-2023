...
func main() {
	...
	fmt.Println(" maxEnergizedCount = ", maxEnergizedCount(internal.FilterOutEmptyLines(lines)))
}
...
func (c *contraption) lightIt(enteringBeam beam) *contraption {
	// round := 0
	paths := []beam{enteringBeam}
	for len(paths) > 0 {
		b := paths[0]
		paths = paths[1:]
...
func maxEnergizedCount(lines []string) int {
	maxE := 0
	for r := range lines {
		maxE = max(maxE, readContraption(lines).lightIt(beam{position: point{horizontal: 0, vertical: r}, direction: '>'}).energizedCount())
		maxE = max(maxE, readContraption(lines).lightIt(beam{position: point{horizontal: len(lines[0]) - 1, vertical: r}, direction: '<'}).energizedCount())
	}

	for c := range lines[0] {
		maxE = max(maxE, readContraption(lines).lightIt(beam{position: point{horizontal: c, vertical: 0}, direction: 'v'}).energizedCount())
		maxE = max(maxE, readContraption(lines).lightIt(beam{position: point{horizontal: c, vertical: len(lines) - 1}, direction: '^'}).energizedCount())
	}
	return maxE
}
